...
3. 房主扫描回传码；Runtime 校验小队 ID、令牌、房主 ID 和有效期后应用 Answer。这次扫描本身就是房主对加入者的许可。
4. 两台手机直接建立 DataChannel，不启动局域网监听端口，也没有第三方信令服务。
...
- 手机双扫首版是一对一连接；连接建立前不要切出 Obsidian，二维码超过 10 分钟需重新创建。
